...
def optimize_nubia(input_path, output_path):
    print(f"\nOptimizing Nubia: {input_path}")
    gltf = pygltflib.GLTF2().load(input_path)
    orig_size = os.path.getsize(input_path)

    # Keep only the first 2 primary loop animations or strip unneeded ones
    if len(gltf.animations) > 2:
        gltf.animations = gltf.animations[:2]
    ...
    print(f"Nubia reduced from {orig_size / (1024*1024):.2f} MB -> {new_size / (1024*1024):.2f} MB ({new_size / 1024:.1f} KB)")

if __name__ == '__main__':
    optimize_nubia('public/nubia__multiversus.glb', 'public/nubia__multiversus.glb')
