...
def compress_image_bytes(raw_bytes, mime_type, max_dim=512, quality=75):
    try:
...
def compress_glb_to_kb(input_path, output_path, max_dim=512, quality=75, strip_tangents=True):
    print(f"\n=======================================================")
    print(f"Compressing to KB: {input_path}")
    ...
    # If strip_tangents is True, remove TANGENT attribute to save ~33% geometry buffer space
    if strip_tangents:
        for mesh in gltf.meshes:
            for prim in mesh.primitives:
                if prim.attributes and prim.attributes.TANGENT is not None:
    ...
    print(f"Compressed {input_path}: {orig_size / 1024:.1f} KB -> {new_size / 1024:.1f} KB ({reduction:.1f}% reduction)")

if __name__ == '__main__':
    targets = [
        ('public/joker_batman_arkham_origins.glb', 512, 75),
        ('public/joker_school_uniform_high_poly.glb', 512, 75),
        ('public/evil_joker_3d_model.glb', 512, 75),
        ('public/batman_origins_suit_-_textured_and_rigged.glb', 384, 70),
        ('public/harley_quin_-_textured_and_rigged.glb', 384, 70),
        ('public/nubia__multiversus.glb', 384, 70),
        ('public/spider_gwen_marvel_ultimate_alliance_3.glb', 512, 75),
        ('public/Door/metal_door_collection.glb', 512, 75),
        ('public/Door/dist/metal_door_collection.glb', 512, 75),
        ('public/Door/public/metal_door_collection.glb', 512, 75),
        ('public/3d_testing_props/modern_office.glb', 512, 75),
    ]

    for path, max_dim, quality in targets:
        if os.path.exists(path):
            compress_glb_to_kb(path, path, max_dim=max_dim, quality=quality, strip_tangents=True)
