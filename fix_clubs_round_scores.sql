-- Fix clubs_round_scores table and RLS policies
-- Run this to fix permission issues

-- Drop existing policies if they exist
DROP POLICY IF EXISTS "Allow authenticated users to read round scores" ON clubs_round_scores;
DROP POLICY IF EXISTS "Allow authenticated users to insert round scores" ON clubs_round_scores;
DROP POLICY IF EXISTS "Allow authenticated users to update round scores" ON clubs_round_scores;

-- Recreate the table (this will preserve data if it exists)
...
-- Policy: Allow all authenticated users to update (needed for upsert)
CREATE POLICY "Allow authenticated users to update round scores"
ON clubs_round_scores FOR UPDATE
TO authenticated
USING (true)
WITH CHECK (true);

-- Grant permissions to authenticated users
GRANT ALL ON clubs_round_scores TO authenticated;
