        ...
        if (nextPhase === 'briefing') {
            ...
            if (endReason === 'master_defeat') {
                ...
                finalParticipants = finalParticipants.map((p: any) => {
                    // Update: User requested +500 for ALL players if Master loses.
                    if (p.role !== 'master') {
                ...
                finalParticipants = finalParticipants.map((p: any) => {
                    if (p.role !== 'master') {
                        // Apply -200 penalty to ALL players
                        // DUPLICATE CHECK: If they were JUST eliminated in the 'result' phase (which runs before this),
                        // 'currentState' might reflect that.
                        // However, 'latestStateForEnd' fetched from DB should be authoritative.
                        // If they are 'eliminated' in DB, they already took the -400 hit for the round.
                        // Does 'Master Win' penalty stack? "if master wins ... -200 for all player".
                        // Yes, it implies an ADDITIONAL penalty for the global loss.
