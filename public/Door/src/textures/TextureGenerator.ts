...
export class TextureGenerator {
  /**
   * Generates crisp, realistic silver-white architectural wall textures.
   */
  public static createSilverWallTexture(): { map: THREE.CanvasTexture; normalMap: THREE.CanvasTexture; roughnessMap: THREE.CanvasTexture } {
    ...
    // Pure white / silver architectural plaster background
    ...
    // Subtle realistic wall paneling grid lines
    ...
    nCtx.strokeStyle = 'rgb(90, 90, 255)';
    nCtx.lineWidth = 3;
    for (let x = 0; x <= 1024; x += panelSize) {
  ...
  /**
   * Generates pure white polished marble floor textures matching the walls.
   */
  public static createSilverFloorTexture(): { map: THREE.CanvasTexture; normalMap: THREE.CanvasTexture; roughnessMap: THREE.CanvasTexture } {
    ...
    // Pure polished white marble background
    ...
    nCtx.strokeStyle = 'rgb(100, 100, 255)';
    nCtx.lineWidth = 4;
    for (let x = 0; x <= 1024; x += tileSize) {
      nCtx.beginPath(); nCtx.moveTo(x, 0); nCtx.lineTo(x, 1024); nCtx.stroke();
    }
    ...
    // Roughness Canvas (Smooth polished finish)
    const rCanvas = document.createElement('canvas');
    rCanvas.width = 1024;
    rCanvas.height = 1024;
    const rCtx = rCanvas.getContext('2d')!;
  ...
  public static createModernDoorTexture(): { map: THREE.CanvasTexture; normalMap: THREE.CanvasTexture; roughnessMap: THREE.CanvasTexture } {
    ...
    // Modern Charcoal & Stainless Steel Composite Door Finish
    ctx.fillStyle = '#0f172a';
    ctx.fillRect(0, 0, 512, 1024);

    // Horizontal Inlay Strips
